...
>
> **状态（2026-06）**：Phase 1 + Phase 2（成功率 / 平均耗时 / Claude agent 分组 / MCP server 钻取）均已实现并落地。
> 本文已对齐实际实现（类型/文件名/刷新接入），并补入实测新结论：① Codex 自 2025/12 起原生支持 Skills，但**无离散调用事件**，
> 只能靠读取 `SKILL.md` 启发式计数；② 排行同分项采用「次数降序→名称升序」稳定排序；③ 成功率/耗时**分母独立**——
> 只对真正提供该信号的样本计入，缺信号来源（如 Claude 无逐工具计时）不入分母，绝不把均值/成功率拉低；
> ④ Claude 用同文件 `tool_use`↔`tool_result` 配对得成功率（`is_error`），用 `isSidechain`/`subagents/` 路径分 main/subagent，
> 并读取 `agent-<id>.meta.json` 的 `agentType` 把子代理细分到具体类型（Explore / Plan / …）。
> 下文凡与早期规划不同处均以「实现」为准（旧规划名保留在括注中以便对照 git 历史）。
...
| Phase 2 | 成功率、耗时（仅有数据的源）、Claude subagent 分组、按 server 钻取 | ✅ 已实现（见 §10；Codex 普通工具耗时仍留待后续） |
...
| 成功率 | ✅ Phase 2 | OpenCode `state.status`、Codex MCP `result.Ok/Err`、Claude 同文件 `tool_use`↔`tool_result`（`is_error`）；Codex 普通工具暂不计 |
| 平均耗时 | ✅ Phase 2 | OpenCode `state.time`、Codex MCP `duration`；Claude 无逐工具计时 → 留白 |
| 规则命中次数 | ❌ 不实现 | 原理无信号；改为「规则清单/状态」可选展示 |
| 按 agent 分组 | ✅ Phase 2 | 仅 Claude：主会话 + 各具体子代理类型（Explore/Plan/…，取自 `agent-<id>.meta.json` 的 `agentType`；缺失则归通用 subagent）。其它来源无此概念 |

## 4. 规范化中间模型（实现）

所有源归一化到统一**聚合条目**，集中在 `QuotaBackend/.../CallAnalytics/CallAnalyticsModels.swift`。
Phase 1 产出「按 日×来源×类别×名称(×server) 聚合好的计数条目」；Phase 2 在此之上追加
**成功率 / 平均耗时的可累加分量**（分母独立）与 **agent 维度**（Claude main/subagent）。
...
/// 一条按「日 × 来源 × 类别 × 名称(× MCP server)(× agent)」聚合后的调用计数。
public struct CallAnalyticsEntry: Codable, Sendable, Hashable {
    ...
    public let agent: String?    // 仅 Claude："main" / 具体子代理类型（Explore/Plan…）/ 兜底 "subagent"；其余来源 nil
    public let dayKey: String    // yyyy-MM-dd（本地时区）
    public var count: Int
    // Phase 2：成功率 / 平均耗时分量。分母独立——只对提供该信号的样本计入。
    public var outcomeKnownCount: Int   // 有成功/失败信号的样本数（成功率分母）
    public var successCount: Int          // 其中成功数（成功率分子）
    public var durationSampleCount: Int  // 有耗时的样本数（均值分母）
    public var durationMsTotal: Double    // 耗时总和（ms）
    public var successRate: Double? { outcomeKnownCount > 0 ? Double(successCount)/Double(outcomeKnownCount) : nil }
    public var avgDurationMs: Double? { durationSampleCount > 0 ? durationMsTotal/Double(durationSampleCount) : nil }
}
```
...
├─ Top-N 排行        CallAnalyticsView+Rankings.swift：lens 分段 + 列含义图例 + 自定义 Capsule 条形 + 成功率/耗时徽章 + 来源圆点（仅「全部」范围）
├─ 按 Agent 分组     CallAnalyticsView.swift `agentCard`：仅 Claude 有子代理活动时显示；主会话置顶、子代理「↳」缩进
└─ 零调用卡          CallAnalyticsView+ZeroCall.swift：技能/MCP 两组芯片（已用绿 / 可清理橙），FlowLayout 自适应
```

排行榜与 Agent 卡的可读性细节（实现）：
- **列含义图例**（`rankingLegend`）：卡顶一行说明「▮ 调用次数 · % 成功率 · ● 来源」，按上下文自适应——无成功率信号的维度不显示「%」段，非「全部」范围不显示来源段。
- **来源圆点仅「全部」范围显示**：单应用筛选（claude/codex/opencode）时所有圆点同色、零信息量，故整列隐藏；仅 `scope == .all` 时显示，用于区分调用来自哪个应用。
- **Agent 卡层级**（`agentRow` + `agentStyle`）：主会话齐左、加粗、条更高；子代理整体右移、带「↳」线索、字号/条高更小，凸显从属。图标按类型名关键词匹配（explore→放大镜、plan→列表、review→印章…），未匹配的新类型兜底 `person.2`；颜色一律按类型名 FNV-1a 哈希从 8 色板取，保证同名恒色、跨重启稳定。
...
- **Phase 1（MVP）✅ 已实现**：三家 MCP/Skill/工具计数（Codex skill 启发式）+ Top-N 排行（稳定排序）+ 每日趋势 + 零调用检测（可清理仅限用户技能）；不做规则命中。
- **Phase 2 ✅ 已实现**：
  - 成功率：OpenCode `state.status`、Codex MCP `result.Ok/Err`、Claude 同文件 `tool_use`↔`tool_result`（`is_error`，缺省/未配对不计入分母）；Codex 普通工具暂不计。
  - 平均耗时：OpenCode `state.time`、Codex MCP `duration`；Claude 无逐工具计时 → 留白。
  - Claude agent 分组：主会话 + 各具体子代理类型（取 `agent-<id>.meta.json` 的 `agentType`，缺失归通用 subagent），UI 单列「按 Agent 分组」卡（仅有 subagent 活动时显示，主会话恒置顶）。
  - MCP server 钻取：排行点 server 展开其具体 tool（含各自次数/成功率/耗时）。
  - 排行行按需显示「成功率 · 平均耗时」徽章（无信号留白；技能/工具维度通常无 → 不占位）。
  - **每应用按自身优势定制**：不强行三家对齐，缺信号的指标在该来源显式留白而非显示 0。
- **Phase 2 余项（待后续）**：Codex 普通工具耗时（call↔output 时间戳推算）、可选「跨工具同名技能合并」、可选「(path,size,mtime) 增量缓存」。
...
8. **成功率/耗时分母独立（Phase 2）** → 只对真正提供该信号的样本计入分母（`outcomeKnownCount` / `durationSampleCount`），缺信号来源不入分母；UI 无数据显式留白，不显示 0%/0ms。
9. **Claude 成功率口径（Phase 2）** → 同文件按 `tool_use_id` 配对 `tool_result`，`is_error==true` 为失败，`false`/缺省为成功；未配到 tool_result（含大行被 4MB 截断、跨文件）→ 成功率未知、不计入分母。
10. **Claude agent 分组（Phase 2）** → 文件路径含 `/subagents/` 判为子代理，并读取同名边车 `agent-<id>.meta.json` 的 `agentType` 细分到具体类型（Explore/Plan/…，拿不到归通用 `subagent`）；常规文件按行内 `isSidechain==true` 兜底为 subagent，否则 main。schema 升 v3（成功率/耗时）→ v4（agent）自动失效旧缓存。
...
- **成功率/耗时覆盖不均（Phase 2）**：OpenCode 全类别有 status+time；Codex 仅 MCP 有 result+duration（普通工具暂不计）；Claude 仅成功率（配对 tool_result）、无耗时。缺信号处一律留白，不补 0。
- **Claude 成功率为部分样本**：大行 tool_result 被 4MB 截断或跨文件未配对的调用，成功率未知、不计入分母；故 Claude 成功率分母可能小于其总调用数。
- **Claude 子代理类型依赖边车文件**：具体类型取自 `agent-<id>.meta.json` 的 `agentType`；老会话或缺边车时该子代理只能归通用 `subagent`。旧格式（主文件 `isSidechain` 行）与新格式（`subagents/` 独立文件）并存时沿用既有计数口径。

## 14. 排障速查（症状 → 责任代码 → 原始信号 → 验证）

出现「数字不对/不显示/对不上」时，按下表从**症状**定位到**责任文件**，再用**验证命令**在真实日志里核对原始信号，区分「解析 bug」与「数据本身如此」。先看缓存口径（见末条），很多“没刷新”其实是缓存命中。

| 症状 | 责任代码 | 原始信号 / 口径 | 本机验证命令 |
|------|----------|------------------|--------------|
| 某 MCP/技能/工具**次数**偏多或偏少 | 对应 `*CallEventSource.classify/parseLine` + `CallAnalyticsLineReader` 的子串预筛与 `maxLineBytes` 上限 | Claude `tool_use.name`；Codex `function_call.name`/`mcp_tool_call_end`；OpenCode `part.data.tool` | Claude：`rg -o '"name":"[^"]+"' ~/.claude/projects/**/*.jsonl \| sort \| uniq -c`；Codex：`rg -o '"name":"[^"]+"' ~/.codex/sessions/**/*.jsonl` |
| **成功率** 0%/100% 看着不对 | `CallEventAccumulator.add(success:)`（分母独立）+ 各源 `outcome` 解析 | Claude `tool_result.is_error`；Codex MCP `result.Ok\|Err`；OpenCode `state.status` | Claude：`rg -o '"is_error":(true\|false)' <file> \| sort \| uniq -c`；先确认是否**小样本**（n=1 时 0/100% 属正常，非 bug） |
| 成功率**整列不显示** | `rankingCard` 的 `showMetrics`（无任一行有 `successRate` 即不显示） | 该 lens/scope 下所有条目 `outcomeKnownCount==0` | 切到有成功率信号的来源（OpenCode/Codex MCP/Claude）验证 |
| **平均耗时**留白 | `add(durationMs:)`；Claude 无逐工具计时、Codex 普通工具未计 | OpenCode `state.time.{start,end}`；Codex MCP `duration{secs,nanos}` | OpenCode part：`state.time`；Codex：`rg -o '"duration":\{[^}]*\}' <file>` |
| **Agent 卡不出现** | `hasSubagentActivity`（需存在 `agent != nil && != "main"` 且 count>0 的 Claude 条目） | 仅 Claude 有 agent 维度；无子代理活动则整卡隐藏 | `find ~/.claude/projects -type d -name subagents` 看是否有子代理会话 |
| 子代理显示「子代理」而非**具体类型**（Explore/Plan） | `ClaudeCallEventSource.readSubagentType`（读边车 `agentType`） | `subagents/agent-<id>.meta.json` 的 `agentType` | `cat ~/.claude/projects/**/subagents/agent-*.meta.json`，确认有无 `agentType` |
| **僵尸技能数与列表对不上** / 跟筛选不一致 | `CallAnalyticsDerived.skillStatuses/serverStatuses` + KPI 同口径（§7）；`CallAnalyticsInventory` 清单探测 | 已装清单 ∩ 实际调用，**按 scope 归属**；可清理仅用户自建技能 | 切「全部」vs 单应用对比；查清单目录：`ls ~/.claude/skills ~/.codex/skills ~/.config/opencode/skills` |
| **MCP server 切错**（下划线处） | `CallAnalyticsNaming` + OpenCode 最长前缀匹配回退 | Claude `mcp__a__b__c`→server=a；OpenCode `<server>_<tool>` 用已装 server 名最长前缀 | 对照 `~/.config/opencode/opencode.json` 的 `mcp` 段确认 server 名 |
| **来源圆点消失** | `rankingRow` 的 `if scope == .all`（设计行为，非 bug） | 单应用范围隐藏来源点 | 切到「全部」范围圆点应恢复 |
| 数字**跨重启不变/改了代码不生效** | `CallAnalyticsStore` 缓存读写；`schemaVersion` 不匹配才重建 | 缓存 `~/.config/aiusage/cache/call-analytics-v<schema>.json` | 改了解析口径但没升 schema 时：删缓存或点刷新；`ls ~/.config/aiusage/cache/call-analytics-*.json` |

排障要点：
- **先分清「解析 bug」还是「数据如此」**：成功率/耗时/次数都先用上表的 `rg` 命令在原始日志里数一遍，再判断 UI 是否错。小样本（单次调用）下 0%/100% 是统计正常现象。
- **缓存优先级最高**：UI 读的是落盘快照；改了解析逻辑务必同步升 `CallAnalyticsSnapshot.currentSchemaVersion`（否则旧缓存继续命中，看似“没生效”）。手动点刷新会强制重算。
- **分母独立是有意为之**：成功率/耗时只统计有该信号的样本，缺信号来源不入分母（不是丢数据）；所以 Claude 成功率分母可能 < 总调用数、Claude 耗时永远留白。
