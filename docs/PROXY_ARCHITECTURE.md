...
- **唯一数据源 = 代理日志**（Claude 家族节点：`anthropicDirect` + `openaiProxy`）。`ProxyRequestLog.estimatedCostUSD` 在请求发生时已用当时节点定价算好并冻结写入（见 `ProxyViewModel+ProxyServer.swift`），天然支持「同模型不同节点不同价」+「历史不可篡改」。
- 删除 JSONL 老管线（`ClaudeProvider` 扫描 / 归档 / 文件缓存 / `proxy-pricing.json` 同步）。
- **永久日归档**喂养仪表盘热力图 / 顶部卡 / 用量统计页。
- 注意：proxy-log 永久归档自本功能上线起累积；早于原始日志保留期且已被裁剪的历史无法找回（proxy-only 设计的固有取舍）。

### Codex（双轨，零重叠）

Codex 会话日志 `session_meta.model_provider` 可区分来源：`aiusage-proxy` = 走代理，`openai`/空 = 订阅/直连（见 `CodexCostProvider+Pricing.sourceTaggedModel`）。但它区分不出「哪个代理节点」（所有代理节点都写同一个 `aiusage-proxy`）。故：

- **API 轨** = 代理日志（Codex 家族节点，按节点冻结价）。JSONL 里的 `(API)` 行丢弃，避免与代理日志重复计算。
- **订阅轨** = JSONL 非代理行，按「订阅定价表（模型→价，账号无关、全局一张）」**逐日冻结**进永久归档（改表只影响未冻结的新数据，历史不变）。Codex 会话日志无账号标识，故订阅定价不区分账号。
- 统计页：API 计费 / 订阅 两轨分列 + 合计。
...
| `~/.config/aiusage/usage-archive/proxy-usage-codex-v1.json` | Codex API 轨永久日归档（同上） |
...
| `QuotaBackend/.../CodexCostProvider+ProxyArchive.swift` | 读 `proxy-usage-codex` 归档 → API 轨日桶（模型加 ` (API)` 标签、成本冻结） |
| `QuotaBackend/.../CodexCostProvider+Tracks.swift` | 订阅轨构建（JSONL `(Sub)` 行**仅统计 token、成本恒 0**，丢弃 `(API)` 行防双计）+ 两轨按日合并 |
| `QuotaBackend/.../CodexSubscriptionUsageArchiveStore.swift` | 订阅轨永久日归档（按 home 一张）：今天之前冻结、今天重算（冻结 token 用量） |
...
- [x] **阶段 3（已作废）**：曾实现 Codex 订阅定价表 + 编辑 UI；后续按「订阅制不按 token 计费」修正为**订阅去价格化**——删除 `CodexSubscriptionPricing.swift` / `CodexSubscriptionPricingSection.swift` / `CodexCostProvider+SubscriptionPricing.swift`，订阅轨仅统计 token、成本恒 0（见下方「订阅去价格化」）
- [x] **阶段 4**：Codex 双轨落地 —— 订阅轨（JSONL 非代理行按订阅表重定价、逐日冻结进 `CodexSubscriptionUsageArchiveStore`，今天之前冻结/今天重算）+ API 轨（读 `proxy-usage-codex` 归档，模型 ` (API)` 标签、成本逐条冻结）；`CodexCostProvider.fetchUsage` 重写为「扫 JSONL → 订阅轨冻结 → 读 API 归档 → 两轨按日合并」，`extra` 增加 `overall.api.*` / `overall.sub.*`，`source.type = codex-two-track`
- [x] **订阅去价格化**：订阅制不按 token 计费 → 删除整套订阅定价（`CodexSubscriptionPricing*` / `+SubscriptionPricing` + Codex 代理页「订阅定价」区）；`buildSubscriptionDays` 只统计 token、`(Sub)` 成本恒 0；合计成本 = 仅 API 轨，`overall.sub.estimatedCostUsd=0`；订阅模型不再算「未定价」；订阅 token 仍逐日冻结进归档（删本地日志不丢）。QuotaBackend 测试 10/10 通过
- [x] **阶段 5**：统计页 / 热力图按 API / 订阅 两轨 + 合计呈现（`UsageTrack` 枚举 + `CostSummary.filtered(by:)`，Codex 家族显示「合计 / API / 订阅」切换器，单轨剖去 ` (API)` / ` (Sub)` 后缀；热力图按轨过滤日总量与 tooltip）；已清理废弃逻辑 —— 删除 `CodexUsageArchiveStore`（旧 `pricingSignature` 重算归档）及 `CodexUsageArchiveState` / `CodexUsageArchive.pricingSignature` 字段；删除 `proxy-pricing.json` 同步（`ProxyViewModel.syncPricingOverrides` / `ProxyRuntimeService.write|clearPricingOverrides` 及相关错误枚举）—— 代理成本一律由 App 端按节点 `pricingForModel` 在请求时冻结，无人再读该文件
