...
- Claude Desktop 有独立的 3P profile、client key、可选目录模式、路由表、配置事务和 `client_surface=desktop` 归因。
...
4. Claude Desktop 可在“固定 Opus/Sonnet/Haiku 智能路由”和“当前节点完整真实模型目录”之间选择；两者都不暴露不被 Desktop 接受的原始 route ID。
5. 切换 AIUsage 活动节点后，Claude Code 与 Desktop 同时切换；智能路由模式不重启 Desktop，完整目录模式只在可见目录变化时自动重载 Desktop。
...
5. 修改 `inferenceModels` 后必须重启还是会热刷新？当前架构不依赖目录热刷新：智能路由的三条 route 固定，普通节点切换只更新 Gateway；完整目录和 `supports1m` 等可见能力发生变化时，AIUsage 主动重载正在运行的 Desktop。
...
除第 5 项已经通过架构解耦外，其余问题没有实证前，相关 ADR 保持 `PROVISIONAL`，不得假装已经确定。
...
- admin 热切换已经能原子替换上游；智能路由无需重写 profile，完整目录则在上游切换后刷新 profile 并按需重载 Desktop。
...
```json
{
  ...
  "inferenceModels": [
    { "name": "claude-opus-4-6-aiusage-v1", "labelOverride": "AIUsage Opus" },
    { "name": "claude-sonnet-4-6-aiusage-v1", "labelOverride": "AIUsage Sonnet" },
    { "name": "claude-haiku-4-5-aiusage-v1", "labelOverride": "AIUsage Haiku" }
  ]
}
```
...
任一步初次写入或校验失败：按快照恢复全部文件，并重新读取确认哈希一致。正常断开时，如果共享文件未变化，走原字节精确恢复；如果 Claude Desktop 在运行期改写了无关偏好，则只恢复 AIUsage 拥有的字段，保留这些新偏好。
...
- 此时“断开”不会覆盖当前第三方 profile；用户再次明确点击“一键接入”则视为授权接管，并把当前 profile 重新设为新的恢复点。
- 不能用整份共享配置文件的哈希变化推断“其它工具接管”：Claude Desktop 自己会改写偏好字段。所有权只看 `appliedId` 和 AIUsage 自有字段。
...
6. 模型 route 必须是 Desktop 可接受的安全客户端身份；智能模式的三条身份跨节点稳定，完整目录模式按真实模型稳定派生。上游模型与客户端 route 不能混为一谈。
...
- 智能路由下节点热切换不改 profile、不重启 Desktop；完整目录下 Gateway 先切换，再刷新 profile 并重载正在运行的 Desktop。
