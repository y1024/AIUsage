...
3. 可选：点 `模型设置`，查看完整 Display name / Model ID，并为上游确实支持 1M 上下文的模型开启 `1M`。
4. 点 `一键接入 Desktop`。
...
| 已保护 | 检测到其它工具接管 profile，AIUsage 停止覆盖 |
...
| 字段 | 示例 | 用途 |
|---|---|---|
| Model ID | `claude-sonnet-4-6-aiusage-v1-15694200761241336191` | Desktop 发给本地 Gateway 的安全路由 |
| Display name | `codex-auto-review` | Desktop 模型选择器展示的真实模型名 |
| Upstream model | `codex-auto-review` | AIUsage 实际转发给节点的模型 |
...
因此 AIUsage 使用 Claude 角色/版本形态加纯数字稳定标识；真实模型名只进入 `labelOverride`、`display_name` 和内部精确映射。

## 4. 逐模型 1M 开关

在 `模型设置` 中，每个真实上游模型都有独立的 `1M` 开关。设置按“节点 + 真实模型 ID”保存，并同步到：
...
3. 新生成的第三方 Model ID 不应包含真实 upstream slug，只应类似 `claude-sonnet-4-6-aiusage-v1-<纯数字>`。
4. Display name 仍应显示真实模型名；不要通过改 Display name 来修路由。
