...
> 状态：CPA 网关产品化重构已实现；本文以当前功能分支代码为准
> 更新日期：2026-07-12
> 上游核对基线：`router-for-me/CLIProxyAPI` 发布版 `v7.2.67`（`2075f77c`）
> 最近一次官方完整资产 live 回归：`v7.2.68`，2026-07-12；路由与安全源码引用仍固定到上面的可复核提交。
...
- 概览的账号、模型、已接入应用、CPA 版本改为紧凑指标，并加入合并 CPA OpenAI、Anthropic、Gemini 三种实时视图、可搜索和复制模型 ID 的协议化模型目录；
...
`cliproxyapi.imageset` 是 AIUsage 为“CPA 网关”设计的产品标识，不宣称是上游 CLIProxyAPI 官方 Logo。该资源通过 `ProviderIconView("cliproxyapi")` 在侧边栏、页头、添加向导和状态卡中保持一致；具体服务商继续复用 Codex、Claude、Antigravity、Kimi、Gemini 等现有 Provider Icons。
...
- 实时模型目录：合并默认 OpenAI `/v1/models`、带 `Anthropic-Version` 的 Anthropic `/v1/models` 与 Gemini `/v1beta/models`，按模型 ID 去重并标注协议；
- 页面打开时立即读取，运行中每 30 秒轻量刷新；账号操作与页面轮询共享按 CPA PID 隔离的单飞任务，重启不会复用旧进程响应；单协议暂时失败时保留成功结果和上次目录，并显示 partial/stale 状态；
- 模型较多时提供模型 ID、Provider 与协议搜索，默认控制展示密度，可展开全部，并支持复制单个模型 ID；
...
- stdout/stderr 持续 drain，脱敏后写入有界内存日志。
...
| model catalog | OpenAI/Anthropic/Gemini 三视图解码、去重、协议标签、30 秒刷新、PID 单飞、partial/stale/空态 | 概览只显示 CPA 实际返回的模型，受管 Provider 仍使用 OpenAI 列表；重启不复用旧响应 |
...
6. 分别读取 OpenAI、Anthropic、Gemini 模型视图，验证协议目录合并不改变受管 Responses Provider 使用的 OpenAI 模型列表；
...
- 900×600、扩大侧边栏、宽窗口下四区导航、账号分组和添加向导不截断；
- 四个概览指标保持紧凑；模型目录的加载、离线、无模型、搜索无结果、展开全部和复制 ID 状态都可辨认；
