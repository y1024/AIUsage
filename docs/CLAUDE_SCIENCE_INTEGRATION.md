...
| `ScienceAuthProxy.swift` + `+Helpers.swift` | 8765 反向代理：兼容型 nonce→cookie 初始化（CLI 回退）、动态 cookie 注入、localhost authority 改写、`/login`→302、HTTP/WS 转发、401 重铸、脱敏自探 |
...
- [x] `ScienceAuthProxy` 反向代理（兼容型 nonce→cookie / CLI 回退 / 动态 cookie / localhost authority / WS 隧道 / 脱敏自探）
