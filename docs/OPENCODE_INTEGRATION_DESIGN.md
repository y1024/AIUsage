...
## 7. Phase 2 设计（多账户 + 配置切换器）

> 实现前需再过一轮寸止确认，此处定方向。

### 7.1 多账户：auth.json 快照切换

- 模式完全镜像 Codex 订阅账号切换（`ProviderActivationManager.activateCodexAccount`）：
  - 候选发现：`ProviderAuthCandidateDiscovery` 加 `opencodeCandidates()`，扫 `~/.local/share/opencode/auth.json`。
  - 快照存储：`AccountCredentialStore`（Keychain）存整份 auth.json 文本，`authMethod: .authFile`。
  - 切换：备份现 auth.json → 写入选中快照 → `activeProviderAccountIds["opencode"]` 标记。提示用户重启 OpenCode 进程。
- 注意：auth.json 是「每 provider 单凭据」的字典，所以一份快照 = 一个「账户组合」。UI 文案按「配置快照」而非「单账号」表达更准确。

### 7.2 配置切换器：opencode.json 受管块

- `OpenCodeConfigManager`（镜像 `CodexConfigManager` 的 backup-as-source-of-truth）：
  - 注入受管键 `provider["aiusage-managed"]`（`npm: "@ai-sdk/openai-compatible"` + baseURL/apiKey/models）+ 顶层 `model` 指向受管 provider；备份 `opencode.json.aiusage.bak`；还原即整文件回滚。
  - JSON 用结构化读写；检测到 `.jsonc`（或解析失败）时拒绝接管并提示，不破坏用户注释。
- 节点模型挂入 `NodeProfile` 体系需新 `NodeType.opencodeConfig`（无代理进程，纯配置写入，激活流程跳过 QuotaServer spawn）——这是对 `ProxyViewModel` 激活管线的最小侵入路径，细化设计放 Phase 2 文档。
