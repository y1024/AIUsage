...
enum ClaudeDesktopProfileTransactionRegression {
    ...
    private static func externalEditScenario(root: URL, fileManager: FileManager) throws {
        ...
        do {
            _ = try store.disconnect()
            throw RegressionFailure.assertion("disconnect overwrote an externally edited profile")
        } catch ClaudeDesktopProfileError.profileChangedExternally(_) {
            let after = try jsonObject(at: paths.profile)
            try require(after["externalField"] as? String == "must-survive", "external profile edit was overwritten")
        }
    }
